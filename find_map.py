...
# note: this is a modified version of pymc's find_MAP function that allows for arbitrary optimization functions for derivative-free optimization.
...
############## Wrappers for various optimization packages
...
def fit_model_map(pm_model, params_to_fit=None, **map_kwargs):
    ...
    if params_to_fit is None:
        params_to_fit = ['ktr', 'gam', 'slope_ven_blast', 'slope_aza_blast', 'slope_ven_wbc', 'slope_aza_wbc', 'B', 'Xl1']
    map_result = find_MAP(model=pm_model, **map_kwargs)
    map_params = [float(map_result[k]) for k in params_to_fit]
    return map_params


def pybobyqa_wrapper(obj, x0, *args, **kwargs):
    import pybobyqa
    if 'bounds' in kwargs:
        bounds = kwargs['bounds']
    else:
        param_names_bounds_m4b = [(0, 1), (0, 1), (0, 1), (0, 2), (0, 1), (0, 2), (0, 5), (0, 5), (0, 5), (0, 1), (0, 1)]
        bounds = param_names_bounds_m4b + [(0, 10), (0, 10)]
    # lol i have basically no idea how the transformation to get the actual x0 is done
    if 'bounds_correction' in kwargs:
    ...
    if 'maxfun' in kwargs:
        ...
        maxfun = 50000
    output = pybobyqa.solve(obj, x0, bounds=bounds_tuple, seek_global_minimum=True, scaling_within_bounds=True, maxfun=maxfun)
    print(output)
    res = OptimizeResult(x=output.x, success=True, message=output.msg, fun=output.f, nfev=output.nf)
    return res
